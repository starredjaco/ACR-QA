...
def generate_report(run_id=None, output_file=None):
    ...
    report_lines.append("")
    report_lines.append(f"- 🔴 **High (Critical):** {len(findings_by_severity['high'])}")
    report_lines.append(f"- 🟡 **Medium (Warning):** {len(findings_by_severity['medium'])}")
    ...
    if findings_by_severity["high"]:
        ...
        for i, finding in enumerate(findings_by_severity["high"], 1):
            ...
            report_lines.append("")
            report_lines.append(f"**Tool:** {finding['tool']} | **Rule:** [{canonical_id}](config/rules.yml)")
            report_lines.append("")
            ...
            if rule_def.get("remediation"):
                ...
                if rule_def.get("example_good"):
                    ...
                    report_lines.append("<summary>📝 Example Fix (click to expand)</summary>")
                    report_lines.append("")
    ...
    if findings_by_severity["medium"]:
        ...
        for i, finding in enumerate(findings_by_severity["medium"][:10], 1):  # Limit to 10
            canonical_id = finding.get("canonical_rule_id", finding["rule_id"])
