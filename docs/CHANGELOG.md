...
## [Unreleased] — 2026-07-02 · License change: MIT → GNU AGPL-3.0 (dual-licensed)

- **Relicensed from MIT to GNU AGPL-3.0.** ACR-QA stays true **open source** (OSI-approved), but with
  copyleft reciprocity: anyone who distributes it or runs a modified version as a network service must
  release their **full source under the AGPL**, which keeps every derivative open and prevents the
  project being taken closed-source. The author retains copyright and offers **commercial licenses**
  (dual licensing) for proprietary use without the source-sharing obligation. Liability disclaimed
  (as-is). Updated `LICENSE` (verbatim AGPL-3.0), README badge + License/dual-license section,
  `pyproject.toml` (+ `acrqa-mcp`, vscode-extension) metadata + OSI classifier, and the
  competitive-baseline table.
...
- **`scripts/realvuln_reliable_recall.py`** — measures per-run recall variance + cost across the 3
  runs RealVuln ships per LLM. **Correctness note:** an earlier framing ("#1 on reliable recall")
  compared ACR-QA's single run to each LLM's 3-run *intersection* — an unfair handicap (the
  intersection is mechanically ≤ any single run). Corrected: the honest "scan once" number is a
  single run's recall. Against each LLM's *worst* single run, **ACR-QA wins only 5/10** — the top
  tier (GPT-5.5, Opus 4.6/4.8, Kimi) out-recalls it even on their unluckiest run.
- **Honest position:** ACR-QA's recall is mid-frontier (ties Opus 4.8 & Gemini, beats Grok/Qwen/
  DeepSeek, trails GPT-5.5/Opus-4.6/Kimi). It does **not** have the highest recall. Its uncontested
  edge is **determinism**: same finding set every run (LLMs: 48–77% stable), at **$0** vs up to $62
  — which matters for auditability, scan diffing, and CI gate stability, not for raw bug count.
...
- **Supply Chain page: intentional empty state.** When a run has no dependency manifest the page
  showed a bare "No dependencies found". Replaced with an explanatory panel (the sample vendors no
