  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \begin{enumerate}\small
        ...
        \item RealVuln (real apps): \good{25--31\%} -- still \emph{beats} Bandit \& Semgrep; 52\% of that corpus is authz/IDOR no SAST can reach
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      \begin{enumerate}\small
        ...
        \item \good{Exploit verification} -- fire a live PoC in a sandbox (10 categories)
        \item \good{Verified Remediation} -- re-run the exploit after the fix; ship only if it now fails
        \item ECDSA + Rekor-signed provenance \& remediation chain
        \item Pre-registered 20-CVE battery (8/8 detectable = 100\%)
      \end{enumerate}
      \vspace{6pt}
      \begin{exampleblock}{The claim no other tool can make}
        Every confirmed finding is \textbf{exploit-proven}; every fix is
        \textbf{re-proven by re-exploitation} and \textbf{signed}.\\
        Snyk retests \emph{statically}. We retest by \emph{exploitation}.
      \end{exampleblock}
    \end{column}
  \end{columns}
\end{frame}
