...
ACR-QA v2.5: Language-Agnostic Code Review Platform}}
...
\midrule
Project Name & ACR-QA v2.5 (Automated Code Review \& Quality Assurance) \\
...
ACR-QA v2.5 is a language-agnostic, on-premises code review platform that automatically detects bad practices, security vulnerabilities, design anti-patterns, and style violations in pull requests. It uses Retrieval-Augmented Generation (RAG) with Cerebras AI to provide evidence-grounded, natural language explanations that help developers understand and fix issues. The MVP focuses on analyzing pull request diffs only, not entire repositories, to keep scope feasible for an 8-month academic project. The system is packaged as a production-ready, Docker-based pipeline with Prometheus metrics, Grafana dashboards, k6 load testing validation, and Pydantic schema validation, aligning with modern backend/DevOps observability and reliability practices.
...
\subsection{Core Innovation (v2.5 Differentiators)}

ACR-QA v2.5 delivers 30+ production-grade features across 4 core innovations:
...
\begin{enumerate}
    ...
    \item \textbf{Quality Gates \& Configuration:} Configurable thresholds block PRs, inline suppression (\texttt{\# acr-qa:ignore}), per-repo YAML config, exit codes for CI/CD integration
\end{enumerate}
...
\toprule
\textbf{Feature} & \textbf{ACR-QA v2.5} & \textbf{SonarQube} & \textbf{CodeClimate} & \textbf{Ruff/ESLint} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{ACR-QA vs Industry Tools (bold = newly implemented in v2.5)}
\end{table}
...
\textbf{Test Coverage:} 97 tests across 6 test suites, runtime 6.59 seconds
...
\textbf{Acceptance:}
\begin{itemize}[leftmargin=*]
    \item[$\checkmark$] 70\%+ code coverage (97 tests)
    \item[$\checkmark$] All tests pass on CI/CD
    \item[$\checkmark$] Test runtime $<$7 seconds
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Test Coverage:} 97 unit tests across 6 test suites with pytest-cov achieving 70\% threshold
\end{itemize}
...
\textbf{E. Unit Tests}
\begin{itemize}[leftmargin=*]
    \item[$\checkmark$] Code coverage $\geq$70\% (97 tests, pytest report)
    \item[$\checkmark$] All tests pass on CI/CD
    \item[$\checkmark$] Runtime $<$7 seconds
