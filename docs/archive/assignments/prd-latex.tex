...
ACR-QA v2.7 is a language-agnostic, on-premises code review platform that automatically detects bad practices, security vulnerabilities, design anti-patterns, and style violations in pull requests. It uses Retrieval-Augmented Generation (RAG) with Groq AI to provide evidence-grounded, natural language explanations that help developers understand and fix issues. The MVP focuses on analyzing pull request diffs only, not entire repositories, to keep scope feasible for an 8-month academic project. The system is packaged as a production-ready, Docker-based pipeline with Prometheus metrics, Grafana dashboards, k6 load testing validation, and Pydantic schema validation, aligning with modern backend/DevOps observability and reliability practices. v2.7 adds competitive features including OWASP compliance reporting, test gap analysis, confidence-based noise control, feedback-driven severity tuning, and a policy-as-code engine.
...
\textbf{Description:} Generate natural language explanations using Groq LLM with evidence-grounding. Explanations include \texttt{[RULE-ID](config/rules.yml)} citations and ``How to Fix'' remediation.
...
\begin{enumerate}
    ...
    \item Call Groq API (llama3.1-8b, temperature=0.3, max tokens=150)
...
\begin{itemize}[leftmargin=*]
    ...
    \item Cost tracking per PR (Groq token count)
...
\begin{itemize}[leftmargin=*]
    ...
    \begin{itemize}
        ...
        \item \texttt{groq\_api\_latency\_ms} (histogram)
    \end{itemize}
...
\textbf{Objective:} Prevent cascading failures when external APIs (Groq, GitHub) fail.

\textbf{Implementation:}
\begin{itemize}[leftmargin=*]
    \item Monitor Groq API response times and error rates
...
def call_groq_api(prompt):
    response = groq_api.explain(prompt)
    return response

# Usage:
try:
    explanation = call_groq_api(prompt)
...
\begin{itemize}[leftmargin=*]
    ...
    \item Prometheus metric: \texttt{circuit\_breaker\_state\{service="groq"\}}
...
def get_explanation(rule_id, code_snippet):
    ...
    # Cache miss: call LLM
    explanation = groq_api.explain(prompt)
...
\midrule
Groq API timeout & Can't generate LLM explanation & Use template explanation \\
Groq API rate limit & Can't generate explanations & Queue job, retry later \\
...
\textbf{Key findings:}
\begin{itemize}[leftmargin=*]
    \item Current monthly cost: \$0.03 (Groq API only)
...
(Concurrency Control) & $\leq$60 GitHub API/hour & (Redis) \\
 & $\leq$50 Groq API/hour & Prevents API quota \\
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{LLM Data:} Code snippets sent to Groq API (documented in privacy policy)
...
- Enforce: $\leq$50 Groq API calls/hour \\
$\downarrow$ \\
...
\midrule
LLM API & Groq (llama3.1-8b) & Free tier, 60-70$\times$ faster than OpenAI, \$0.60/1M tokens \\
\midrule
...
\midrule
Rate Limiting & Redis Token Bucket Algorithm & Handle GitHub/Groq limits \\
\midrule
...
\begin{itemize}[leftmargin=*]
    ...
    \item[$\checkmark$] Recovery successful after 30s timeout + probe succeeds
    \item[$\checkmark$] Prometheus metric updates: \texttt{circuit\_breaker\_state\{service="groq"\}}
\end{itemize}
...
\midrule
\endhead
Groq API downtime & Medium & High & Template fallback; store all prompts for replay \\
\midrule
