...
**Latest Run:** May 15, 2026 · **Version:** v3.9.5
**Unit Tests:** 2,160 passed, 43 skipped · **Coverage:** 84.89% · **0 warnings**
...
`FRONTEND.api.*` and `FRONTEND.auth.*` are suppressed because the FastAPI dependency-injection pattern uses runtime-evaluated type annotations that mypy cannot resolve without `--explicit-package-bases`. The auth and API code is covered by functional and integration tests.
