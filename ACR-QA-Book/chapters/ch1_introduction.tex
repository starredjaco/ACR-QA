...
\begin{table}[H]
\caption{Development Phases --- \ACRQA{} v5.0.0rc2}
\label{tab:phases}
\centering
...
Chapter~2 surveys the state of the art in static analysis, AI-assisted code review, RAG, and cryptographic inventory. Chapter~3 details the system architecture: twelve-stage pipeline, canonical schema, and database design. Chapter~4 covers the implementation. Chapter~5 presents the full evaluation across four corpora: Layer~A ground truth (97.1\% / 100\% security-class precision), Layer~B 30-repo adversarial corpus (Confirmed Tier 96.4\% conservative / 100\% optimistic, 95\% CI [90.9\%, 100\%]), 20-CVE recall battery (100\% on detectable), and five novel evaluations (X1 live-CVE, X2 exploit verification with 13 categories, X3 AI-generated code, X4 time-travel backtest, X5 LLM-augmented detection), plus a head-to-head against Bandit and Semgrep where \ACRQA{} reaches F\textsubscript{1}~98.2\%. Chapter~6 summarises contributions, lessons learned, limitations, and future directions.
