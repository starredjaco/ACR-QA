...
Validation spans four corpora: (i) Layer~A ground truth (DVPWA, Pygoat, VulPy, DSVW) — \textbf{97.1\% overall / 100\% security-class precision} on 836 findings; (ii) Layer~B adversarial 30-repo precision corpus — \textbf{96.4\% conservative / 100\% optimistic precision (P4 Confirmed Tier)}, 95\% CI [90.9\%, 100\%], 55 findings; (iii) 20-CVE recall battery — \textbf{8/8 detectable = 100\% recall}; (iv) head-to-head — \textbf{F\textsubscript{1} 98.2\%} vs.\ Bandit 21.8\% and Semgrep 45.7\%. Four novel evaluations (X1 live-CVE blind holdout, X2 detect~$\to$~exploit~$\to$~fix~$\to$~re-exploit verification across 13 categories with Verified Remediation attestation, X3 AI-generated code study, X4 time-travel predictive backtest) round out the empirical contribution. The \textbf{2,897-test} suite reaches 84.89\% CORE coverage, \textbf{9/10} OWASP Top~10 categories are detected, and \textbf{500\,RPS} is sustained at $<$2\,s p99. The platform ships as \texttt{acrqa==5.0.0rc2} on PyPI with Docker Compose, Helm, and Terraform deployments at zero recurring cost.
