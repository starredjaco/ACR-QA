...
\begin{enumerate}
  ...
  \item \textbf{Novel Empirical Battery (X1--X4):} Four publishable evaluations: a live-CVE blind holdout (2024--2025 CVEs not in the original recall set); detect $\to$ Docker-sandbox exploit $\to$ patch verification across SQLi, command injection, and SSTI; an AI-generated code vulnerability study (400 samples, 4 LLMs) finding 8--12$\times$ the human baseline of 7.1 findings/KLOC; and a time-aware predictive backtest on 10 Django checkpoints. Together they directly answer the committee questions ``did you overfit?'', ``are the findings exploitable?'', and ``does the tool generalise to AI-written code?''
...
The current release of \ACRQA{} v5.0.0rc1 has the following acknowledged limitations:
