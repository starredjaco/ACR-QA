...
\begin{figure}[p]
  \centering
  \includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{arch_overview}
...
The core of \ACRQA{} is a twelve-stage sequential processing pipeline. Each stage receives the output of the preceding stage and produces an enriched artefact consumed by the next, as described in the subsections below.
...
The scoring pipeline is detailed quantitatively in Chapter~\ref{ch:testing} (Figure~\ref{fig:confusion_matrix_plot} and Table~\ref{tab:confusion_matrix}).
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{rag_engine}
...
The taint analyser performs intra-procedural data-flow analysis within each Python function.
For each \texttt{FunctionDef} node, a \texttt{\_FunctionTaintVisitor} tracks five HTTP-input
source patterns (\texttt{request.args}, \texttt{request.form}, \texttt{request.json},
\texttt{request.\allowbreak cookies}, \texttt{os.environ}) propagating through assignment chains,
f-strings, and string concatenations, until reaching one of three sink types:
SQL \texttt{execute()} calls, \texttt{eval()}/\texttt{exec()}, and \texttt{subprocess} invocations.
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{er_diagram}
  \caption{Database Entity-Relationship Diagram (13-Table PostgreSQL Schema, v5.0.0rc2)}
  \label{fig:db_schema}
\end{figure}
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{docker_stack}
...
Version 5.0.0 extends the core pipeline with four new analytical engines, each addressing a gap identified after v4.6.0 evaluation. Each engine is described in the subsections below with its integration points into the core pipeline and database tables.
