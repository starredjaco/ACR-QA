...
\begin{figure}[htbp]
  ...
  \caption{High-Level Architecture of the \ACRQA{} System (v4.0.0)}
  \label{fig:architecture_overview}
\end{figure}

The six service layers — FastAPI API \cite{fastapi_framework}, React dashboard, Celery workers \cite{celery_project}, CORE engine, PostgreSQL \cite{postgresql_docs}, and the \texttt{config/rules.yml} knowledge base — communicate via a private Docker bridge network. Authentication \cite{fielding2000rest}, Prometheus \cite{prometheus_docs} instrumentation, and Pydantic v2 validation are applied at the API layer; all analysis executes asynchronously on Celery workers.
...
\begin{table}[H]
\caption{Tool Execution Mapping and Typical Runtime (v4.0.0)}
...
\subsection{Stage 5 — Supply Chain Analysis}

Stage 5 runs two parallel sub-analyses. The \textbf{SCA} scanner parses Python \texttt{requirements.txt} and Node.js \texttt{package-lock.json}, querying an offline OSV snapshot \cite{osv_database} ($\sim$1.5\,GB, updated weekly) to report dependency CVEs with CVSS scores and affected version ranges. The \textbf{CBoM} scanner performs AST-based detection of cryptographic API usage, classifying 62 algorithms as NIST Approved, Deprecated, or PQC-Vulnerable, and outputs a CycloneDX CBoM JSON document.

\subsection{Stage 6 — Quality Gate Enforcement}

Stage 6 evaluates results against configurable quality gate rules defined in \texttt{.acrqa.yml} and produces a binary pass/fail verdict. The gate fails on any CRITICAL finding with $C_s \geq 70$, more than five HIGH findings at that threshold, more than three security-class findings with $C_s \geq 50$, or any hardcoded secret above $C_s = 50$. PQC-vulnerable or deprecated cryptographic algorithms in the CBoM trigger warnings rather than failures. All remaining configurations yield a pass.

\subsection{Stage 7 — Structured Output Delivery}

Stage 7 serialises the complete analysis result. The REST API exposes four output formats:
JSON (full structured result), SARIF~2.1 \cite{sarif_standard} (GitHub Advanced Security integration), HTML self-contained report, and PDF compliance report. Findings are also posted as inline PR review comments to the triggering GitHub pull request, if a \texttt{GITHUB\_TOKEN} is configured.
...
\begin{figure}[htbp]
  ...
  \caption{Database Entity-Relationship Diagram (10-Table Schema, v4.0.0)}
  \label{fig:db_schema}
\end{figure}
...
The FastAPI application exposes 32 endpoints grouped into four resource namespaces. Table~\ref{tab:api_endpoints} summarises the principal endpoints.

\begin{table}[H]
\caption{Principal FastAPI REST Endpoints (v4.0.0)}
...
\begin{figure}[htbp]
  ...
  \caption{Docker Compose Service Topology (7 Services, v4.0.0)}
...
Version 5.0.0 extends the core pipeline with four new analytical engines, each addressing a gap identified after v4.0.0 evaluation.
...
This chapter has presented the complete system design of \ACRQA{} v5.0.0b1, including the twelve-stage processing pipeline, the canonical finding schema, the confidence scoring function, the taint analyser, the call-graph reachability engine, the RAG-and-entropy enrichment mechanism, the CBoM module, the 10-table database schema, the 52-endpoint FastAPI surface, the 7-service containerised deployment architecture, and the four new v5.0.0 engines (Heuristic Risk Predictor, Time-Travel Analyzer, IaC Scanner, PR Risk Score). Chapter~\ref{ch:implementation} details the implementation of each component.
